...
immatrix = array([array(Image.open('C:\\Python\\Scripts\\images\\input_test_resized' + '\\' + im2)).flatten() for im2 in imlist],'f')                
...
model.load_weights('C:\\Users\\LEECH\\cnn-weight')
Y_pred = model.predict(X_test)
Y= np.reshape(Y_pred,(3))
y_pred = np.argmax(Y_pred, axis=1)
if y_pred == 0:
    print(round(Y[0]*100,2), "% 확률로 경미한 알츠하이머 증상이 있습니다.")
elif y_pred==1:
    print(round(Y[1]*100,2), "% 확률로 중증 알츠하이머 증상이 있습니다.")
elif y_pred==2:
    print(round(Y[2]*100,2), "% 확률로 정상입니다.")
